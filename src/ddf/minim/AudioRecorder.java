...
/**
 * An
 * <code>AudioRecorder</code> can be used to record audio that is being played
 * by a
 * <code>Recordable</code> object. An
 * <code>AudioRecorder</code> need not necessarily record to disk, but the
 * recorders you receive from
 ...
 * can constructor an
 * <code>AudioRecorder</code> directly, using an absolute path for the filename
 * (such as "C:\My Documents\Music\song.wav"). You can also create a recorder
 * that uses your own implementation of {@link SampleRecorder} (like if you
 * wanted to implement an mp3 encoder).
 *
...
public class AudioRecorder {
    ...
    /**
     * Constructs an
     * <code>AudioRecorder</code> that will use
     * <code>recorder</code> to record
     * <code>recordSource</code>.
     *
     * @param recordSource the <code>Recordable</code> object to record
     * @param recorder the <code>SampleRecorder</code> to use to record it
     */
     ...
     * <code>SampleRecorder</code> to save. This will only work if you have
     * called {@link #endRecord()}. If this was created with a buffered
     * recorder, then calling {@link #beginRecord()} after saving will not
     * overwrite the file on the disk, unless this method is subsequently
     ...
     * (this will always be the case if you use
     * <code>createRecorder</code> or the first constructor for
     * <code>AudioRecorder</code>).
     *
     ...
     * record source, then add the new record source, also muted, and then
     * unmute the new record source. Otherwise, you'll probably wind up with a
     * pop in the recording.
     *
    ...
    /**
     * Sets the
     * <code>SampleRecorder</code> for this recorder. Similar caveats apply as
     * with {@link #setRecordSource(Recordable)}. This calls
     * <code>endRecord</code> and
     * <code>save</code> on the current
     * <code>SampleRecorder</code> before setting the new one.
     *
     * @param recorder the new <code>SampleRecorder</code> to use
     */
