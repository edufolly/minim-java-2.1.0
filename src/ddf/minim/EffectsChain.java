...
/**
 * An
 * <code>EffectsChain</code> is a list of {@link AudioEffect AudioEffects} that
 * gives you the ability to enable and disable effects, as you would in a
 * typical DAW. When you add an effect, it is added to the end of the chain and
 * is enabled. When you remove an effect, effects further down the chain are
 * moved up a slot.
 * <code>EffectsChain</code> is itself an
 * <code>AudioEffect</code>, so you can easily create groups of effects that can
 * be enabled/disabled together by putting them in an
 * <code>EffectsChain</code> and then adding that chain to an
 * <code>Effectable</code> as a single effect.
 * <code>EffectsChain</code> is fully
 * <code>synchronized</code> so that it is not possible to add and remove
...
public class EffectsChain implements AudioEffect {
    ...
    /**
     * Adds
     * <code>e</code> to the end of the chain.
     *
     ...
     * <code>i</code><sup>th</sup> effect from the chain.
     *
    ...
    /**
     * Gets the
     * <code>i<sup>th</sup></code> effect in the chain.
     *
    ...
    /**
     * Enables
     * <code>e</code> if it is in the chain.
     *
    ...
    /**
     * Sends
     * <code>samp</code> to each effect in the chain, in order.
     *
     ...
     * Sends
     * <code>sampL</code> and
     * <code>sampR</code> to each effect in the chain, in order. The two float
     * arrays should correspond to the left and right channels of a stereo
     * signal.
     *
