...
/**
 * An
 * <code>AudioSample</code> is a special kind of file playback that allows you
 * to repeatedly <i>trigger</i> an audio file. It does this by keeping the
 ...
 * available and
 * <code>AudioEffect</code>s can be attached to it, but there are not the cueing
 * abilities found on an
 * <code>AudioSnippet</code> and
 * <code>AudioPlayer</code>. All you can do is
 * <code>trigger()</code> the sound. However, you can trigger the sound even if
 * it is still playing back. It is not advised that you use this class for long
 * sounds (like entire songs, for example) because the entire file is kept in
 * memory.
 *
