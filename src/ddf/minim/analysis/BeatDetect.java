 ...
 * sound's energy history, that is to say if the brain detects a <em>brutal
 * variation in sound energy</em>. Therefore if the ear intercepts a monotonous
 * sound with sometimes big energy peaks it will detect beats, however, if you
 * play a continuous loud sound you will not perceive any beats. Thus, the beats
 * are big variations of sound energy. </blockquote> In fact, the two algorithms
 * in this class are based on two algorithms described in that paper. <p> To use
 * this class, inside of
 * <code>draw()</code> you must first call
 * <code>detect()</code>, passing the
 * <code>AudioBuffer</code> you want to analyze. You may then use the
 * <code>isXXX</code> functions to find out what beats have occurred in that
 * frame. For example, you might use
 * <code>isKick()</code> to cause a circle to pulse. <p> BeatDetect has two
 * modes: sound energy tracking and frequency energy tracking. In sound energy
 * mode, the level of the buffer, as returned by
 ...
 * divided into average bands using
 * <code>logAverages()</code>, and each of these bands is tracked individually.
 * The result is that it is possible to track sounds that occur in different
 * parts of the frequency spectrum independently (like the kick drum and snare
 * drum). <p> In sound energy mode you use
 * <code>isOnset()</code> to query the algorithm and in frequency energy mode
 * you use
 * <code>isOnset(int i)</code>,
 * <code>isKick()</code>,
 * <code>isSnare()</code>, and
 * <code>isRange()</code> to query particular frequnecy bands or ranges of
 * frequency bands. It should be noted that
 * <code>isKick()</code>,
 * <code>isSnare()</code>, and
 * <code>isHat()</code> merely call
 ...
 * use
 * <code>isRange()</code> directly to locate the bands that provide the most
...
public class BeatDetect {
     ...
     * <code>timeSize</code> and
     * <code>sampleRate</code> will be set to 1024 and 44100, respectively, so
     * that it is possible to switch into FREQ_ENERGY mode with meaningful
     * values.
     *
     */
    public BeatDetect() {
     ...
     *
     * @param timeSize the size of the buffer
     * @param sampleRate the sample rate of the samples in the buffer
     */
     ...
     *
     * @param algo either BeatDetect.SOUND_ENERGY or BeatDetect.FREQ_ENERGY
     */
     ...
     * <code>ab</code>. This is a cumulative process, so you must call this
     * function every frame.
     *
     * @param ab the AudioBuffer to analyze.
     */
    ...
    /**
     * Analyze the samples in
     * <code>buffer</code>. This is a cumulative process, so you must call this
     * function every frame.
     *
     ...
     * the
     * <code>i<sup>th</sup></code> frequency band. In sound energy mode this
     * always returns false.
     *
     * @param i the frequency band to query
     ...
     *
     * @param low the index of the lower band
     * @param high the index of the higher band
     * @param threshold the smallest number of bands in the range
     * <code>[low, high]</code> that need to have registered a beat for this to
     * return true
     * @return true if at least <code>threshold</code> bands of the bands
     * included in the range <code>[low, high]</code> have registered a beat
     */
     ...
     * the algorithm will wait for
     * <code>s</code> milliseconds before allowing another beat to be reported.
     * You can use this to dampen the algorithm if it is giving too many
     * false-positives. The default value is 10, which is essentially no
     * damping. If you try to set the sensitivity to a negative value, an error
     * will be reported and it will be set to 10 instead.
     *
     * @param s the sensitivity in milliseconds
     */
     ...
     * use with
     * <code>inRange()</code> if the provided drum detecting functions aren't
     * what you need or aren't working well.
     *
     * @param p the PApplet to draw in
     */
