 ...
 *
 * <p> <b>Windowing</b> <p> Windowing is the process of shaping the audio
 * samples before transforming them to the frequency domain. The Fourier
 * Transform assumes the sample buffer is is a repetitive signal, if a sample
 * buffer is not truly periodic within the measured interval sharp
 * discontinuities may arise that can introduce spectral leakage. Spectral
 * leakage is the speading of signal energy across multiple FFT bins. This
 * "spreading" can drown out narrow band signals and hinder detection. <p> A <a
 ...
 * <code>WindowFunction</code> will call this method to apply the window to a
 * sample buffer. The number passed to the method is an offset within the length
 * of the window curve.
 *
