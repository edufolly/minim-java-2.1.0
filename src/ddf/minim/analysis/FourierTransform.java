 ...
 * <code>N/2</code> frequency bands in the spectrum. <p> As an example, if you
 * construct a FourierTransform with a
 ...
 * frequency, which in this case is 22050, so the bandwidth is equal to about 50
 ...
 * band in the spectrum. <p> <b>Usage</b> <p> A typical usage of a
 * FourierTransform is to analyze a signal so that the frequency spectrum may be
 * represented in some way, typically with vertical lines. You could do this in
 * Processing with the following code, where
 ...
 * </pre>
 *
 * <b>Windowing</b> <p> Windowing is the process of shaping the audio samples
 * before transforming them to the frequency domain. The Fourier Transform
 * assumes the sample buffer is is a repetitive signal, if a sample buffer is
 * not truly periodic within the measured interval sharp discontinuities may
 * arise that can introduce spectral leakage. Spectral leakage is the speading
 * of signal energy across multiple FFT bins. This "spreading" can drown out
 * narrow band signals and hinder detection. </p> <p> A <a
 * href="http://en.wikipedia.org/wiki/Window_function">windowing function</a>
 ...
 * <p> <b>Averages</b> <p> FourierTransform also has functions that allow you to
 * request the creation of an average spectrum. An average spectrum is simply a
 * spectrum with fewer bands than the full spectrum where each average band is
 * the average of the amplitudes of some number of contiguous frequency bands in
 * the full spectrum. <p>
 ...
 * each average will span 8 bands of the full spectrum. <p>
 ...
 * averages because they map more directly to how humans perceive sound. <p>
 ...
 * average their amplitudes for you. <p> If you don't want any averages
 * calculated, then you can call
 ...
 * <code>forward()</code>. <p> <b>Inverse Transform</b> <p> FourierTransform
 * also supports taking the inverse transform of a spectrum. This means that a
 * frequency spectrum will be transformed into a time domain signal and placed
 * in a provided sample buffer. The length of the time domain signal will be
...
public abstract class FourierTransform {
    ...
    /**
     * A constant indicating no window should be used on sample buffers. Also
     * referred as a <a
     * href="http://en.wikipedia.org/wiki/Window_function#Rectangular_window">Rectangular
     * window</a>.
     */
    public static final WindowFunction NONE = new RectangularWindow();
    /**
     * A constant indicating a <a
     * href="http://en.wikipedia.org/wiki/Window_function#Hamming_window">Hamming
     * window</a> should be used on sample buffers.
     */
    public static final WindowFunction HAMMING = new HammingWindow();
    /**
     * A constant indicating a <a
     * href="http://en.wikipedia.org/wiki/Window_function#Hann_window">Hann
     * window</a> should be used on sample buffers.
     */
    public static final WindowFunction HANN = new HannWindow();
    /**
     * A constant indicating a <a
     * href="http://en.wikipedia.org/wiki/Window_function#Cosine_window">Cosine
     * window</a> should be used on sample buffers.
     */
    public static final WindowFunction COSINE = new CosineWindow();
    /**
     * A constant indicating a <a
     * href="http://en.wikipedia.org/wiki/Window_function#http://en.wikipedia.org/wiki/Window_function#Triangular_window_.28non-zero_end-points.29">Triangular
     * window</a> should be used on sample buffers.
     */
    public static final WindowFunction TRIANGULAR = new TriangularWindow();
    /**
     * A constant indicating a <a
     * href="http://en.wikipedia.org/wiki/Window_function#Bartlett_window_.28zero_valued_end-points.29">Bartlett
     * window</a> should be used on sample buffers.
     */
    public static final WindowFunction BARTLETT = new BartlettWindow();
    /**
     * A constant indicating a <a
     * href="http://en.wikipedia.org/wiki/Window_function#Bartlett.E2.80.93Hann_window">Bartlett-Hann
     * window</a> should be used on sample buffers.
     */
    public static final WindowFunction BARTLETTHANN = new BartlettHannWindow();
    /**
     * A constant indicating a <a
     * href="http://en.wikipedia.org/wiki/Window_function#Lanczos_window">Lanczos
     * window</a> should be used on sample buffers.
     */
